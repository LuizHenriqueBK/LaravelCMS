...
@section('content')
    <div class="row">
        ...
        <header class="col col-md-12 d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h1 mb-0 text-gray-800">{{ Str::ucfirst(__('roles')) }}</h1>
            <a href="{{ route('admin.roles.create') }}"
                class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">
                <i class="fas fa-plus fa-sm text-white-50"></i>
                {{ Str::ucfirst(__('create new role')) }}
            </a>
        </header>
                            ...
                            <tr>
                                ...
                                <td>
                                    @forelse($row->permissions as $permission)
                                        @if($loop->index == 7)
                                            <div class="badge badge-info m-1 p-2">...</div>
                                            @break
                                        @endif
                                        <div class="badge badge-info m-1 p-2">{{ $permission->title }}</div>
                                    @empty
                                    &nbsp;
                                    @endforelse
                                </td>
